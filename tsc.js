...
var _range = (j) => Array.from({ length: j }, (x, i) => i);
function take(list, n) {
    return list.slice(0, n);
}
function rangeTo(size, startAt = 0) {
...
function addChart(series, containerName, chartOptions) {
    ...
    var opts = Object.assign(Object.assign({}, options), chartOptions);
...
(function (LineChartModule) {
    function addLineChart(series, containerName) {
        ...
        var opts = Object.assign(Object.assign({}, options), lineChartOptions);
        var container = $("#" + containerName);
...
const radioButtonCreate = {
    ...
    getAnRadioButton: function getAnRadioButton(id, label, name) {
        ...
        var labelHtml = `<label class='btn btn-outline-primary'  for='${name}${id}'>${label}</label>`;
...
function addSectionToAll(n, selectedValue, formElementKey, containingDiv, includeButton) {
    let div3 = makeRepeatableDiv(n, formElementKey, includeButton);
...
function setupSelectToAll(divId, n, selectedValue, formElementKey) {
    ...
    let sel = makeSelectBox(newId, false);
...
function setupSelect(divId, selectId, isMultiple, set, onChangeToList) {
    let sel = makeSelectBox(selectId, isMultiple);
